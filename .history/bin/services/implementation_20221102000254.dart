...
class NewsImplementation {
  static String getUrlFromParam({String? site = gNewsUrl, String? search}) {
    if (site == nytimesKey) {
      return "$nytimesUrl?q=${search ?? "headline"}&api-key=${env[nytimesKey]}";
    }
    return "$gNewsUrl${search != null ? "search?q=$search&token=${env[gnewsKey]}&lang=en&country=us&max=20" : "top-headlines?lang=en&token=${env[gnewsKey]}"}";
  }
